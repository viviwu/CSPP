...
@property (nonatomic, strong) UIButton           *callHotlineButton;
...
- (void)setupUI {
    ...
    [self addSubview:self.callHotlineButton];
...
- (UIButton*)callHotlineButton{
    if (!_callHotlineButton) {
        _callHotlineButton = [UIButton buttonWithType:UIButtonTypeCustom];
        [_callHotlineButton setImage:[UIImage imageNamed:@"callback"] forState:UIControlStateNormal];
        [_callHotlineButton addTarget:self action:@selector(callHotlineHandler) forControlEvents:UIControlEventTouchUpInside];
    }
    return _callHotlineButton;
}
...
- (void)callHotlineHandler{
    [self makeToast:@"您当前金币不足，无法接通到直播现场" duration:1 position:CSToastPositionCenter];
}
...
- (void)configeConstraints {
    ...
    [self.callHotlineButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.trailing.equalTo(self.mas_trailing).offset(-6);
        make.top.equalTo(self.pictureRotateButton.mas_bottom).offset(30);
        make.size.mas_equalTo(CGSizeMake(50, 50));
    }];
